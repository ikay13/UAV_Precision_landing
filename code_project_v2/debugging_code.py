...
def display_error_and_text(frame, curr_err_px_tuple, alt_from_img, uav_inst):
    ...
    texts = [f"State: {state}", f"Altitude from FC: {altitude:.2f}", f"Altitude from image: {alt_from_img:.2f}"]
    ...
    if curr_err_px != [0, frame.shape[0]]: #Default value meaning object not detected
        ...
        cv.arrowedLine(frame, middle_point, end_point, arrow_color, arrow_thickness, cv.LINE_AA, tipLength=arrow_tip_length)
    else:
        texts.append("Not detected")

    for idx, text in enumerate(texts):
        cv.putText(frame, text, (10, 20+idx*20), cv.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 1, cv.LINE_AA)
